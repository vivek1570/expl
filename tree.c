...
int find_label(struct tnode*lef,struct tnode*rig)
{
    if(lef->nodetype==tSELF)
    {
        return Class_Mlookup(Cptr,rig->varname)->Flabel;
    }
    else if(lef->nodetype==tField)
    {
        return Class_Mlookup(Class_Flookup(Cptr,lef->right->varname)->Ctype,rig->varname)->Flabel;
    }
    else if(lef->nodetype==tVAR)
    {
        return Class_Mlookup(Lookup(lef->varname)->Ctype,rig->varname)->Flabel;
    }
    return -1;
}
...
int codeGen(FILE*fp,struct tnode*root)
{
    ...
    case tFieldFun:
        ...
        fprintf(fp,"PUSH R0\n"); //for self
        int f_label=find_label(root->left,root->right);
        fprintf(fp,"CALL F%d\n",f_label); //call the function or return address allocating
